#include "airsim_ros_wrapper.hpp"
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    gimbal_angle_quat_cmd_sub_ = nh_->create_subscription<stella_sim::msg::GimbalAngleQuatCmd>("~/gimbal_angle_quat_cmd", 50, std::bind(&AirsimROSWrapper::gimbal_angle_quat_cmd_cb, this, _1));
    gimbal_angle_euler_cmd_sub_ = nh_->create_subscription<stella_sim::msg::GimbalAngleEulerCmd>("~/gimbal_angle_euler_cmd", 50, std::bind(&AirsimROSWrapper::gimbal_angle_euler_cmd_cb, this, _1));
    origin_geo_point_pub_ = nh_->create_publisher<stella_sim::msg::GPSYaw>("~/origin_geo_point", 10);
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        vehicle_ros->env_pub_ = nh_->create_publisher<stella_sim::msg::Environment>(topic_prefix + "/environment", 10);
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            std::function<void(const stella_sim::msg::VelCmd::SharedPtr)> fcn_vel_cmd_body_frame_sub = std::bind(&AirsimROSWrapper::vel_cmd_body_frame_cb, this, _1, vehicle_ros->vehicle_name_);
            drone->vel_cmd_body_frame_sub_ = nh_->create_subscription<stella_sim::msg::VelCmd>(topic_prefix + "/vel_cmd_body_frame", 1, fcn_vel_cmd_body_frame_sub); // todo ros::TransportHints().tcpNoDelay();

            std::function<void(const stella_sim::msg::VelCmd::SharedPtr)> fcn_vel_cmd_world_frame_sub = std::bind(&AirsimROSWrapper::vel_cmd_world_frame_cb, this, _1, vehicle_ros->vehicle_name_);
            drone->vel_cmd_world_frame_sub_ = nh_->create_subscription<stella_sim::msg::VelCmd>(topic_prefix + "/vel_cmd_world_frame", 1, fcn_vel_cmd_world_frame_sub);

            std::function<bool(std::shared_ptr<stella_sim::srv::Takeoff::Request>, std::shared_ptr<stella_sim::srv::Takeoff::Response>)> fcn_takeoff_srvr = std::bind(&AirsimROSWrapper::takeoff_srv_cb, this, _1, _2, vehicle_ros->vehicle_name_);
            drone->takeoff_srvr_ = nh_->create_service<stella_sim::srv::Takeoff>(topic_prefix + "/takeoff", fcn_takeoff_srvr);

            std::function<bool(std::shared_ptr<stella_sim::srv::Land::Request>, std::shared_ptr<stella_sim::srv::Land::Response>)> fcn_land_srvr = std::bind(&AirsimROSWrapper::land_srv_cb, this, _1, _2, vehicle_ros->vehicle_name_);
            drone->land_srvr_ = nh_->create_service<stella_sim::srv::Land>(topic_prefix + "/land", fcn_land_srvr);
            ...
            std::function<void(const stella_sim::msg::CarControls::SharedPtr)> fcn_car_cmd_sub = std::bind(&AirsimROSWrapper::car_cmd_cb, this, _1, vehicle_ros->vehicle_name_);
            car->car_cmd_sub_ = nh_->create_subscription<stella_sim::msg::CarControls>(topic_prefix + "/car_cmd", 1, fcn_car_cmd_sub);
            car->car_state_pub_ = nh_->create_publisher<stella_sim::msg::CarState>(topic_prefix + "/car_state", 10);
        }
        ...
        for (auto& curr_sensor_map : vehicle_setting->sensors) {
            ...
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Barometer: {
                    SensorPublisher<stella_sim::msg::Altimeter> sensor_publisher =
                        create_sensor_publisher<stella_sim::msg::Altimeter>("Barometer", sensor_setting->sensor_name, sensor_setting->sensor_type, curr_vehicle_name + "/altimeter/" + sensor_name, 10);
    ...
    if (vehicle_name_ptr_map_.size() > 1 && airsim_mode_ == AIRSIM_MODE::DRONE) {
        takeoff_all_srvr_ = nh_->create_service<stella_sim::srv::Takeoff>("~/all_robots/takeoff", std::bind(&AirsimROSWrapper::takeoff_all_srv_cb, this, _1, _2));
        land_all_srvr_ = nh_->create_service<stella_sim::srv::Land>("~/all_robots/land", std::bind(&AirsimROSWrapper::land_all_srv_cb, this, _1, _2));

        vel_cmd_all_body_frame_sub_ = nh_->create_subscription<stella_sim::msg::VelCmd>("~/all_robots/vel_cmd_body_frame", 1, std::bind(&AirsimROSWrapper::vel_cmd_all_body_frame_cb, this, _1));
        vel_cmd_all_world_frame_sub_ = nh_->create_subscription<stella_sim::msg::VelCmd>("~/all_robots/vel_cmd_world_frame", 1, std::bind(&AirsimROSWrapper::vel_cmd_all_world_frame_cb, this, _1));

        vel_cmd_group_body_frame_sub_ = nh_->create_subscription<stella_sim::msg::VelCmdGroup>("~/group_of_robots/vel_cmd_body_frame", 1, std::bind(&AirsimROSWrapper::vel_cmd_group_body_frame_cb, this, _1));
        vel_cmd_group_world_frame_sub_ = nh_->create_subscription<stella_sim::msg::VelCmdGroup>("~/group_of_robots/vel_cmd_world_frame", 1, std::bind(&AirsimROSWrapper::vel_cmd_group_world_frame_cb, this, _1));

        takeoff_group_srvr_ = nh_->create_service<stella_sim::srv::TakeoffGroup>("~/group_of_robots/takeoff", std::bind(&AirsimROSWrapper::takeoff_group_srv_cb, this, _1, _2));
        land_group_srvr_ = nh_->create_service<stella_sim::srv::LandGroup>("~/group_of_robots/land", std::bind(&AirsimROSWrapper::land_group_srv_cb, this, _1, _2));
    }

    // todo add per vehicle reset in AirLib API
    reset_srvr_ = nh_->create_service<stella_sim::srv::Reset>("~/reset", std::bind(&AirsimROSWrapper::reset_srv_cb, this, _1, _2));
...
bool AirsimROSWrapper::takeoff_srv_cb(std::shared_ptr<stella_sim::srv::Takeoff::Request> request, std::shared_ptr<stella_sim::srv::Takeoff::Response> response, const std::string& vehicle_name)
{
    unused(response);
...
bool AirsimROSWrapper::takeoff_group_srv_cb(std::shared_ptr<stella_sim::srv::TakeoffGroup::Request> request, std::shared_ptr<stella_sim::srv::TakeoffGroup::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
bool AirsimROSWrapper::takeoff_all_srv_cb(std::shared_ptr<stella_sim::srv::Takeoff::Request> request, std::shared_ptr<stella_sim::srv::Takeoff::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
bool AirsimROSWrapper::land_srv_cb(std::shared_ptr<stella_sim::srv::Land::Request> request, std::shared_ptr<stella_sim::srv::Land::Response> response, const std::string& vehicle_name)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
bool AirsimROSWrapper::land_group_srv_cb(std::shared_ptr<stella_sim::srv::LandGroup::Request> request, std::shared_ptr<stella_sim::srv::LandGroup::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
bool AirsimROSWrapper::land_all_srv_cb(std::shared_ptr<stella_sim::srv::Land::Request> request, std::shared_ptr<stella_sim::srv::Land::Response> response)
{
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
bool AirsimROSWrapper::reset_srv_cb(std::shared_ptr<stella_sim::srv::Reset::Request> request, std::shared_ptr<stella_sim::srv::Reset::Response> response)
{
    unused(request);
    unused(response);
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::car_cmd_cb(const stella_sim::msg::CarControls::SharedPtr msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::vel_cmd_body_frame_cb(const stella_sim::msg::VelCmd::SharedPtr msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::vel_cmd_group_body_frame_cb(const stella_sim::msg::VelCmdGroup::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::vel_cmd_all_body_frame_cb(const stella_sim::msg::VelCmd::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::vel_cmd_world_frame_cb(const stella_sim::msg::VelCmd::SharedPtr msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::vel_cmd_group_world_frame_cb(const stella_sim::msg::VelCmdGroup::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::vel_cmd_all_world_frame_cb(const stella_sim::msg::VelCmd::SharedPtr msg)
{
    std::lock_guard<std::mutex> guard(control_mutex_);
...
void AirsimROSWrapper::gimbal_angle_quat_cmd_cb(const stella_sim::msg::GimbalAngleQuatCmd::SharedPtr gimbal_angle_quat_cmd_msg)
{
...
void AirsimROSWrapper::gimbal_angle_euler_cmd_cb(const stella_sim::msg::GimbalAngleEulerCmd::SharedPtr gimbal_angle_euler_cmd_msg)
{
    try {
        tf2::Quaternion quat_control_cmd;
...
stella_sim::msg::CarState AirsimROSWrapper::get_roscarstate_msg_from_car_state(const msr::airlib::CarApiBase::CarState& car_state) const
{
    stella_sim::msg::CarState state_msg;
...
stella_sim::msg::Environment AirsimROSWrapper::get_environment_msg_from_airsim(const msr::airlib::Environment::State& env_data) const
{
    stella_sim::msg::Environment env_msg;
...
stella_sim::msg::Altimeter AirsimROSWrapper::get_altimeter_msg_from_airsim(const msr::airlib::BarometerBase::Output& alt_data) const
{
    stella_sim::msg::Altimeter alt_msg;
...
stella_sim::msg::GPSYaw AirsimROSWrapper::get_gps_msg_from_airsim_geo_point(const msr::airlib::GeoPoint& geo_point) const
{
    stella_sim::msg::GPSYaw gps_msg;
...
VelCmd AirsimROSWrapper::get_airlib_world_vel_cmd(const stella_sim::msg::VelCmd& msg) const
{
    VelCmd vel_cmd;
...
VelCmd AirsimROSWrapper::get_airlib_body_vel_cmd(const stella_sim::msg::VelCmd& msg, const msr::airlib::Quaternionr& airlib_quat) const
{
    VelCmd vel_cmd;
...
rclcpp::Time AirsimROSWrapper::update_state()
{
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            stella_sim::msg::CarState state_msg = get_roscarstate_msg_from_car_state(car->curr_car_state_);
        ...
        stella_sim::msg::Environment env_msg = get_environment_msg_from_airsim(env_data);
...
void AirsimROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        for (auto& sensor_publisher : vehicle_ros->barometer_pubs_) {
            ...
            stella_sim::msg::Altimeter alt_msg = get_altimeter_msg_from_airsim(baro_data);
